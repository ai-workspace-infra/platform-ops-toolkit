...
Run origin policy. Billing's public host is owned by the core Edge Gateway Worker, which proxies to
the declared Cloud Run Billing upstream. The retired `billing_origin_host` field is accepted only
during migration and its old DNS alias is removed; no Cloudflare Origin Ruleset is required. The workflow
renders the topology to a temporary JSON document for existing consumers;
