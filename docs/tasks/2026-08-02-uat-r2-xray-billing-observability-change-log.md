...
| UAT deploy workflow | [Run 30732938304](https://github.com/ai-workspace-infra/platform-ops-toolkit/actions/runs/30732938304) 成功完成 |
...
本次 workflow 的 Monitor Agent、Agent Proxy Services 和 DNS observe job 均已完成并返回成功。部署日志确认两个 Exporter systemd 实例已安装并 active，使用 `v0.6.0-uat.20260802.1`，UAT 监听地址为 `0.0.0.0:8080` / `0.0.0.0:8081`。

但是，Web SaaS reusable workflow 的 `OBSERVE_URLS` 为空，并明确提示：`web-saas/uat has no observe_urls wired — delivery cannot be observed, only requested`。所以 workflow 绿灯只代表请求和 Ansible 部署步骤成功，不能代表 Doco-CD 已完成 reconcile，也不能代表 Billing 已成功拉到 Exporter 快照。

当前从外部探测 `167.179.64.91:8080` 和 `167.179.64.91:8081` 均超时；Playbooks 的 common firewall 当前只保留 22/80/443，没有为 UAT Billing → Exporter 打开 8080/8081 或提供 443 反代入口。这是 Portal 仍显示 0 B 的首要阻断点，需从 console UAT 主机再做一次同源连通性确认。
...
1. 从 `console-uat` 主机验证 `http://agent-proxy.onwalk.net:8080/health` 和 `:8081/health`，确认 Billing 到 Exporter 的网络路径。
2. 检查 Billing 容器实际生效的 `EXPORTER_SOURCES_JSON`，确认不是 loopback，且包含当前 UAT source。
3. 验证 Exporter `/v1/snapshots/latest` / `/v1/snapshots/window` 返回有效快照，再检查 Billing source sync 状态和 PostgreSQL 计费表。
4. 使用 UAT 已配置账户产生一小段可控流量，分别验证 Billing 数据库和 Accounts summary。
5. 刷新 `https://console-uat.onwalk.net/panel/account`，核对用量、余额和周期。
6. 打开 Grafana Xray Dashboard，确认实时监控数据出现；若仍无数据，沿观测链路排查，不回滚计费链路。
7. 将命令输出、页面截图、Grafana 查询结果和最终结论补充到本记录末尾。
