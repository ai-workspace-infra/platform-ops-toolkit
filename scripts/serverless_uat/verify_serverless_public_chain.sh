...
origin="https://${console_host}"
api_origin="https://${accounts_host}"

for ((attempt = 1; attempt <= VERIFY_ATTEMPTS; attempt++)); do
  console_dns="$(dig +short @1.1.1.1 "${console_host}" | sed -n '1p')"
  accounts_dns="$(dig +short @1.1.1.1 "${accounts_host}" | sed -n '1p')"
  ...
  if [[ -n "${console_dns}" && -n "${accounts_dns}" &&
    ...
    echo "Serverless public chain verified: ${console_host} -> ${accounts_host} (CORS preflight 204)"
    exit 0
  fi

  echo "Waiting for serverless public chain (${attempt}/${VERIFY_ATTEMPTS}): console_dns=${console_dns:-missing}, accounts_dns=${accounts_dns:-missing}, console_http=${console_status}, preflight=${preflight_status:-missing}" >&2
  if (( attempt < VERIFY_ATTEMPTS )); then
    sleep "${VERIFY_INTERVAL_SECONDS}"
  fi
done

echo "Serverless public chain verification failed for ${console_host} and ${accounts_host}." >&2
exit 1
