...
BEGIN
  ...
  IF to_regclass('public.bridge_credentials_user_tenant_idx') IS NULL
     OR to_regclass('public.bridge_credentials_active_user_tenant_uk') IS NULL
     OR to_regclass('public.overlay_registrations_owner_created_idx') IS NULL
     OR to_regclass('public.overlay_registrations_network_pending_idx') IS NULL
     OR to_regclass('public.overlay_registrations_network_created_idx') IS NULL
     OR to_regclass('public.overlay_registrations_identity_pending_idx') IS NULL THEN
    RAISE EXCEPTION 'UAT Accounts prior schema indexes do not match reviewed baseline';
  END IF;
  IF EXISTS (
