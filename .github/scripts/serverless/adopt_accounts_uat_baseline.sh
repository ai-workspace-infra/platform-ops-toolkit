...
verified="$(psql "${target_dsn}" -X -Atqc "SELECT version::text || ':' || dirty::text || ':' || (SELECT count(*) FROM information_schema.columns WHERE table_schema='public' AND table_name='users' AND column_name IN ('subscription_valid_from','subscription_valid_until','last_active_at','archived_at'))::text FROM public.schema_migrations" 2>/dev/null)" || fail "Could not verify UAT Accounts baseline."
[[ "${verified}" == "2026091401:false:4" ]] || fail "UAT Accounts baseline verification mismatch."
echo "UAT Accounts expand-only baseline adopted: version 2026091401, subscription columns 4/4."
if [[ -n "${GITHUB_STEP_SUMMARY:-}" ]]; then
  printf 'UAT Accounts expand-only baseline adopted: `2026091401`, subscription columns `4/4`. Existing user values unchanged.\n' >>"${GITHUB_STEP_SUMMARY}"
fi
