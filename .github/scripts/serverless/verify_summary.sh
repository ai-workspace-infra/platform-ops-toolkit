...
for result in "${SUPABASE_RESULT:-}" "${CLOUD_RUN_RESULT:-}" "${CLOUDFLARE_RESULT:-}" "${FRONTEND_ROUTER_RESULT:-}" "${EDGE_GATEWAY_RESULT:-}" "${STATIC_PAGES_RESULT:-}" "${SERVERLESS_DOMAINS_RESULT:-}"; do
  case "${result}" in
    failure|cancelled)
      exit 1
      ;;
