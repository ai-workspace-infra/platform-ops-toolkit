...
for required in \
  ...
  'envs/${DEPLOY_ENV}' \
  'backend-config="key=${{ steps.config.outputs.state_key }}"' \
  'manifest == "resources/xworktech.com/#{environment}/gcp/open-platform-#{environment}.yaml" ? "platform" : File.basename(manifest, ".yaml")' \
  'Terraform apply' \
...
for required in 'cloud_provider:' 'options: [gcp-cloud]' 'options: [none, plan, apply]' 'name: Node stage' 'service_manifest:' 'provider_manifest:' 'vault_env_path: ${{ needs.declaration.outputs.environment }}' 'github_environment: ${{ needs.declaration.outputs.github_environment }}' 'gcp_resource_manifest: ${{ inputs.provider_manifest }}'; do
