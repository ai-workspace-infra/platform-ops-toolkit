...
grep -Fq 'path "kv/data/uat/ulighthost-xconnect/observability.svc.plus"' "${existing_one_policy}"
...
grep -Fq 'kv/data/uat/ulighthost-xconnect/${{ env.ONE_VAULT_KEY }}' "${workflow}"
grep -Fq 'kv/data/uat/ulighthost-xconnect/${{ env.ONE_VAULT_KEY }} sudo_password | ONE_BECOME_PASSWORD' "${workflow}"
...
grep -Fq "frontend: caddy-unix-h2c" "${workflow}"
grep -Fq "listen_socket: /run/xconnect-gateway/xray.sock" "${workflow}"
...
grep -Fq 'ONE_HOST" == "observability.svc.plus' "${existing_one_deploy}"
