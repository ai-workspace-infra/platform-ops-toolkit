...
if [[ -n "${MATRIX_HOST:-}" ]]; then
  mapfile -t agent_hosts < <(
    jq -r --arg host "${MATRIX_HOST}" '[to_entries[] | select(.key == $host and (.value.groups // [] | contains(["agent_proxy"]))) | [.key, .value.ip] | @tsv] | .[]' "${cmdb_file}"
  )
else
  mapfile -t agent_hosts < <(
    jq -r '[to_entries[] | select(.value.groups // [] | contains(["agent_proxy"])) | [.key, .value.ip] | @tsv] | .[]' "${cmdb_file}"
  )
fi
...
for host_entry in "${agent_hosts[@]}"; do
  IFS=$'\t' read -r host_name host_ip <<< "${host_entry}"
  [[ -n "${host_name}" && -n "${host_ip}" ]] || continue

  deadline=$(( $(date +%s) + timeout_seconds ))
  last_failure=""
  while :; do
    if failure="$(check_host "${host_name}" "${host_ip}" 2>&1)"; then
    ...
    if [[ $(date +%s) -ge ${deadline} ]]; then
      ...
      diagnose_host "${host_name}" "${host_ip}"
      exit 1
    fi
    sleep "${poll_seconds}"
  done
