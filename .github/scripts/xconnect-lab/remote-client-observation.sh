...
xconnect status --state-dir "$state" | jq -e --arg device "$client_id" --arg network "$network_id" \
  '.joined == true and .device_id == $device and .network_id == $network and ((.config_id // .revision) | type == "string" and length > 0) and .generations.state > 0 and .runtime.applied == true and .runtime.core_id == "xray" and .credential.present == true and .credential.expired == false' >/dev/null || {
