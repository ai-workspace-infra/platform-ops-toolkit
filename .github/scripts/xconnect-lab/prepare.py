...
def main():
    ...
    if action == 'backend':
        save(folder / 'backend.json', {
            ...
            'use_path_style': True})
        return
    values = {'run_id': run, 'aws_region': spec['aws']['region'],
              'aws_instance_type': spec['aws']['instance_type'],
              'vpc_cidr': spec['aws']['vpc_cidr'],
              'vultr_region': spec['vultr']['region'], 'vultr_plan': spec['vultr']['plan']}
    if action == 'resources':
        ...
        values.update(aws_ami=ami, vultr_os_id=matches[0]['id'], runner_cidr=ip + '/32',
        ...
        allowed = {'aws_vpc.lab', 'aws_subnet.lab', 'aws_internet_gateway.lab', 'aws_route_table.lab',
                   'aws_route_table_association.lab', 'aws_security_group.client', 'aws_key_pair.lab',
                   'aws_instance.client', 'vultr_ssh_key.lab', 'vultr_firewall_group.lab',
                   'vultr_firewall_rule.ssh', 'vultr_firewall_rule.client', 'vultr_firewall_rule.zero', 'vultr_instance.gateway'}
