...
prefix=sit/xconnect-lab/_leases
case "${1:?}" in
  create)
    run="$(<"$LAB_DIR/run-id")"
    jq -n --arg run "$run" --arg iac "$IAC_REF" --arg gitops "$GITOPS_REF" \
      --arg cli "$CLI_REF" --arg xray "$XRAY_REF" \
      --arg expires "$(jq -r .expires_at "$LAB_DIR/variables.json")" \
      '{run:$run,expires_at:$expires,inputs:{mode:"cleanup",cleanup_run:$run,iac_ref:$iac,gitops_ref:$gitops,cli_ref:$cli,xray_ref:$xray}}' > "$LAB_DIR/lease.json"
  ...
  reap)
    ...
    while IFS= read -r key; do
      [[ "$key" =~ ^sit/xconnect-lab/_leases/xcl-[0-9]+-[0-9]+\.json$ ]] || exit 1
      ...
      jq -e '.inputs.mode == "cleanup" and .inputs.cleanup_run == .run and ([.inputs.iac_ref,.inputs.gitops_ref,.inputs.cli_ref,.inputs.xray_ref] | all(test("^[0-9a-f]{40}$")))' "$LAB_DIR/lease.json" >/dev/null
